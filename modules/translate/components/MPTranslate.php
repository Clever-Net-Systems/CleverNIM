...
class MPTranslate extends CApplicationComponent{
	...
	function getLanguage() {
		...
		if(($language=@$this->_cache['language'])!==null)
			return $language;
		elseif(Yii::app()->config->get('langselect') == "Fixed")
			return Yii::app()->config->get('fixedlang');
		elseif(Yii::app()->config->get('langselect') == "User")
			return Yii::app()->user->isGuest ? Yii::app()->config->get('fixedlang') : User::model()->findByPk(Yii::app()->user->id)->lang;
		elseif(Yii::app()->config->get('langselect') == "Browser") {
			$lang = Yii::app()->getRequest()->getPreferredLanguage();
			if (key_exists($lang, Yii::app()->config->get('authorizedlanguages')))
				return $lang;
			/* Look for matching uppercase version of locale */
			if (strpos($lang, "_") !== false) {
				$country = substr($lang, 0, 2);
				$locale = substr($lang, 3, 2);
				$lang = $country . "_" . strtoupper($locale);
			}
			if (key_exists($lang, Yii::app()->config->get('authorizedlanguages')))
				return $lang;
			/* Look for matching language for 2 letter country code */
			if (sizeof($lang) == 1) {
				foreach (Yii::app()->config->get('authorizedlanguages') as $llang) {
					if (strpos($llang, $lang) === 0) {
						return $llang;
					}
				}
			}
	...
	function dropdown() {
		...
		case "Browser":
			return "Your current language is " . $this->getLanguage();
		case "User":
			Yii::app()->getClientScript()->registerScript(self::ID.'-dropdown','
			$("#'.self::ID.'").change(function(){
				$.post("'.Yii::app()->getController()->createUrl("/translate/translate/set").'",
					{"'.self::ID.'":$(this).val()},
