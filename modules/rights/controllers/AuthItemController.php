	...
	*/
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // Allow role Administrateur to access Rights
				'actions'=>array(
					'permissions',
					'operations',
					'tasks',
	...
	*/
	public function actionUpdate()
	{
		...
		if( $childSelectOptions!==array() )
		{
			...
			if( isset($_POST['child'])===true ) {
				foreach ($_POST['child'] as $child => $val) {
					// Add the child and load it
					$this->_authorizer->authManager->addItemChild($itemName, $child);
					$ochild = $this->_authorizer->authManager->getAuthItem($child);
					$ochild = $this->_authorizer->attachAuthItemBehavior($ochild);

					// Set a flash message for adding the child
					Yii::app()->user->setFlash($this->module->flashSuccessKey,
						Rights::t('core', 'Child :name added.', array(':name'=>$ochild->getNameText()))
					);
				}
