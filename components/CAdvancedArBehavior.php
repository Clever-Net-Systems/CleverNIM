...
class CAdvancedArbehavior extends CActiveRecordBehavior
{
	...
	protected function writeManyManyTables() 
	{
		...
		foreach($this->getRelations() as $relation) 
		{
			...
			// Log the new m-m relationship
			/* TODO Optimiser ça: transformer en PDO */
			/* Don't log each row save if we're doing massive import */
			if ($this->Owner->scenario != "import") {
				...
				$log->field = $relation['m2mForeignField'];
				...
				foreach($this->owner->$key as $foreignobject) {
					...
					if (is_numeric($foreignobject)) {
						$model = call_user_func($relation['foreignTable'] . "::model");
						$obj = $model->findByPk($foreignobject);
						if ($obj && isset($obj->_intname)) {
							$foreignobject = $obj->_intname;
						}
					}
					if(!empty($foreignobject))
						$log->new_value .= ' ' . $foreignobject . ' ';
				}
				$log->new_value .= ' ]';
				$log->save();
			}
